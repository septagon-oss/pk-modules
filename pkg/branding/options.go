...
// WithAdminBasePath overrides the admin shell's mount point used when
// building the branding page's route (Task 6). Defaults to "/admin",
// normalized the same way admin.NewShell normalizes its own BasePath.
func WithAdminBasePath(path string) Option {
	return func(c *config) { c.adminBasePath = path }
}
