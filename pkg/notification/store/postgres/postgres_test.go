// postgres_test.go runs the notification_management Postgres adapter through
// the same behavioral checks the sqlite adapter passes — cross-tenant / cross-
// user isolation, the create/list/mark-read round-trip, nullable column
// handling, subscription add/list/remove, ordering, pagination, duplicate-id and
// not-found behavior — against a real Postgres. It is gated on
// PK_POSTGRES_TEST_DSN: with no DSN the tests skip, so `go test ./...` stays
...
// adapter is held to the identical tenant/user-isolation guarantees as the
// embedded one, by executable check rather than by faith.
//
// ADR: ADR-0009 (ports-only module communication), ADR-0029 (file purpose declaration).
// Convention: C-14 (every Go file declares its purpose).
package postgres_test

// Validates: REQ-NOTIF-002, REQ-PORTS-001.
// Per: ADR-0009.
// Discipline: C-14.
import (
	"context"
	...
	"github.com/septagon-oss/pk-modules/pkg/notification/store/postgres"
)
...
// newStore opens the adapter and truncates the module's tables so each test
// starts clean. The tests are serial (no t.Parallel) because they share one
// database.
func newStore(t *testing.T) *postgres.Store {
	...
	if _, err := db.Exec(`TRUNCATE TABLE notifications, notification_subscriptions`); err != nil {
...
// TestNilArguments proves nil-input rejection: New(nil), Create(nil), and
// AddSubscription(nil) all error rather than panic.
...
// TestCreateDuplicate proves a duplicate notification primary key maps to
// store.ErrDuplicate (SQLSTATE 23505 on notifications_pkey).
func TestCreateDuplicate(t *testing.T) {
	s := newStore(t)
	ctx := context.Background()
	if err := s.Create(ctx, notif("dup", "user-1", time.Now().UTC())); err != nil {
		t.Fatalf("Create #1: %v", err)
	}
	if err := s.Create(ctx, notif("dup", "user-1", time.Now().UTC())); !errors.Is(err, store.ErrDuplicate) {
