...
// WithHasher selects the password hasher used by SetPassword/VerifyPassword.
// The default is a bcrypt hasher with cost 12 (passhash.DefaultCost).
func WithHasher(h passhash.Hasher) Option {
	return func(c *config) { c.hasher = h }
}

// WithTenantService wires the tenant service for cross-module validation.
// Optional; when nil, tenant references are not pre-validated by user_management.
