// postgres_test.go exercises the auth_management session Postgres adapter
// against a real Postgres, covering the same surface the sqlite adapter's tests
// cover: the create/get round-trip, IssuedAt defaulting, not-found behavior,
// single and per-user revoke semantics, and primary-key uniqueness. It is gated
// on PK_POSTGRES_TEST_DSN: with no DSN the DB-backed tests skip, so
// `go test ./...` stays green on a machine without Postgres, and CI (or a
// developer with a container) sets the DSN to enforce the contract. auth has no
// tenant-scoped list or update, so there is no AssertListTenantScoped /
// AssertUpdateCannotReassignTenant to run here — the session id is the bearer
// secret and lookups are keyed on it alone, which this test round-trips.
//
...
import (
	...
	"github.com/septagon-oss/pk-modules/pkg/auth/store/postgres"
)
...
func newStore(t *testing.T) *postgres.Store {
	...
	if _, err := db.Exec(`TRUNCATE TABLE auth_sessions`); err != nil {
...
// TestNewRejectsNilDB needs no database, so it runs unconditionally.
...
func TestCreateDuplicate(t *testing.T) {
	...
	// A duplicate primary key is a duplicate session id, not a generic error.
