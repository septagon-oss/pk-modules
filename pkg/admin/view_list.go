...
// view_list.go renders the resource list page by composing pk-ui: SearchBar,
// Button, Table (sortable), Pagination (cursor mode), and EmptyState. The
// DOM is a contract with _admin.js — ids, data-* hooks, and ARIA relations
// are what the script binds — but every component's markup and classes are
// pk-ui's own. The page head keeps its editorial classes per the chrome line
// drawn in view_chrome.go; the only admin-declared styling is layout.
...
func entityListView(data entityListData) g.Node {
	...
	return layout(data.shellView, h.Section(
		...
		h.Div(h.Class(layoutClasses.Toolbar.Compile()), h.Role("search"),
			web.SearchBar(molecules.SearchBarProps{
		...
		web.Table(molecules.TableProps{
		...
		web.Pagination(molecules.PaginationProps{
